...
<body>
        ...
        <?php else: ?>
            <?php foreach ($tasksWithSubtasks as $task): ?>
                <div class="task-container" id="task-<?= $task['id'] ?>">
                    ...
                    <div class="subtask-success-message-<?= $task['id'] ?> message success" style="display:none;">
                    ...
                    <?php elseif (empty($task['team_members']) && $is_logged_in): ?>
                        <p>Please assign members to this task first (in the 'Assign Members' section below).</p>
                    <?php elseif (empty($task['team_members']) && !$is_logged_in): ?>
                         <p>Members not yet assigned to this task. Log in to assign members.</p>
                    <?php else: ?>
                                ...
                                <tbody>
                                    <?php foreach ($task['subtasks'] as $subtask): ?>
                                        <tr>
                                            ...
                                            <td>
                                                <select name="signature[<?= $subtask['id'] ?>]" class="person-select" <?php if (!$is_logged_in) echo 'disabled'; ?>>
            ...
            <table id="taskTable">
                ...
                <tbody>
                    ...
                    <tr data-task-id="<?= $task['id'] ?>">
                        ...
                        <td>
                            <select class="member-select" name="tasks[<?= $task['id'] ?>][member1]" <?php if (!$is_logged_in) echo 'disabled'; ?>>
                        ...
                        <td>
                            <select class="member-select" name="tasks[<?= $task['id'] ?>][member2]" <?php if (!$is_logged_in) echo 'disabled'; ?>>
    ...
    <div class="section" id="reset-section">
        ...
        <div id="resetSignaturesSuccessMessage" class="message success" style="display:none;"></div>
        <button id="resetAllSignaturesButton" class="danger-button">Reset sign-offs</button>
                                
        <h2>Reset supplies</h2>
        <div id="resetSuppliesSuccessMessage" class="message success" style="display:none;"></div>
        <button id="resetSuppliesButton" class="danger-button">Reset supplies</button>
    
        <h2>Reset Late Tasks</h2>
        <div id="resetLateTasksSuccessMessage" class="message success" style="display:none;"></div>
